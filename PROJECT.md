...
- **POS: divisor arrastrable entre productos y cobro** (`sales/pos/page.tsx`). El panel de cobro deja de ser fijo (`lg:w-[420px]`); se arrastra un divisor (pointer events, mouse+táctil), ancho persistido en `localStorage` por PC, doble clic resetea. Mín 360px / máx dejando sitio al panel de productos. Solo escritorio ≥1024px.
- **Fix raíz `/`** (`app/page.tsx`): redirigía siempre a `/login`, botando al usuario con sesión abierta. Ahora `redirect('/dashboard')` y el middleware decide (sin token → login, `mustChangePassword` → cambio de clave, con token → dashboard por rol).
- **CxC nueva: crear/editar clientes inline** (`receivables/new` + nuevo `components/customer-form-modal.tsx`). Modal reutilizable espejo de `supplier-form-modal` (documento V/E/J/G/C/P + SENIAT, teléfono/email/dirección, cupo/días de crédito, contribuyente especial, empresa del grupo, aviso de duplicado). Botones "Nuevo/Editar" junto al selector de Cliente; al guardar refresca la lista y selecciona. **Respeta el permiso de crédito** (`canEditCredit` = ADMIN o `MANAGE_CUSTOMER_CREDIT`): deshabilita Cupo/Días/Autorizado-por + aviso, valida cupo>0 → días y autorizado obligatorios (igual que el form original y el backend).
- **CxP nueva: crear/editar proveedores inline** (`payables/new`), reutilizando el `SupplierFormModal` de la compra. Botones "Nuevo/Editar" junto al selector de Proveedor; al guardar refresca y selecciona. (Los proveedores no tienen gateo de permiso de crédito.)
- **CxC nueva: retención de IVA sufrida** (`receivables/new` + `receivables.service`): opción "Crear retención IVA" (solo fiscal), simétrica a la CxP, con **% y N° de comprobante**. Crea una línea `isRetentionLine` negativa (`documentType 'RETENCION'`) en el libro de ventas con el N° del comprobante. Sin cambio de schema (usa `SalesBookEntry`). Caveat: solo declara al libro, no crea un `CustomerIvaRetention` neteable en el recibo (follow-up si se requiere).
- **CxC nueva: campo "Nro. Documento"** (número de factura manual): se guarda en `Receivable.documentNumber` (columna ya existente, sin migración) y se usa como número de factura en el libro de ventas (`invoiceNumber` + `affectedDocNumber` de la retención), con fallback al correlativo `CXC`.
#### Sesión 69b (2026-07-11) — Módulo "Por despachar" (comandas de retiro con despacho parcial)
Mercancía pagada que el cliente retira después, a veces por partes (bloques/cemento). Reemplaza el folder de tickets térmicos.
- **Schema:** `Dispatch` (1×factura, DSP-XXXX, status auto, `scheduledDate`, contacto/tel editable) · `DispatchItem` (producto + zona + cant. facturada/despachada acumulada) · `DispatchDelivery` (historial de cada retiro). Migración `20260711180000_dispatch_module` + `fix-schema.sql`. **No toca inventario** (stock ya salió al cobrar; es logístico).
- **Backend `DispatchModule`** (`@RequireModule('commands','inventory')`): crear por N° factura (valida pagada, sin servicios, zona por categoría→printArea), lista + búsqueda, vista por artículos de zona (`/dispatches/items`), despacho parcial (`/:id/deliver` acumula `quantityDelivered` + crea `DispatchDelivery`), cancelar.
- **Frontend `/dispatch`** (sidebar COMANDAS): crear (textbox factura), tabs Comandas / Todos / por zona, modal con edición de fecha/contacto, despacho parcial e historial de retiros.

- **Dashboard: KPIs contado vs crédito** (`dashboard.service` `getSalesByPaymentType` + `dashboard/page`): 2 KPIs con ventas de contado y a crédito (monto + N° facturas + Bs, vs período anterior), por `Invoice.paymentType`. Grilla de KPIs a 8 tarjetas (`lg:grid-cols-4`).
- **Dashboard gerencial: KPIs Cashea/Crediagro + línea de facturas** (`dashboard.service` + `dashboard/page`): 2 KPIs nuevos con el monto facturado por Cashea y Crediagro (backend `getFinancingSales` suma pagos por método, con vs período anterior; grilla 4→6). La gráfica de ventas por hora/día suma una 2da línea con la cantidad de facturas (`ComposedChart`, eje derecho; el backend ya devolvía `count`).
- **Dashboard gerencial: KPI "Clientes nuevos" + drill-down** (`dashboard.service` `getNewCustomers` + `gerencial-client`): tarjeta con clientes creados en el período (excluye empresas del grupo, respeta el rango día-Caracas por `createdAt`) con desglose "X compraron (%) · Y sin comprar" (comprado = ≥1 factura `PAID/PARTIAL_RETURN/RETURNED`, vía filtro de relación `some`/`none` — sin listas de IDs, escala sin romper el límite de bind-vars). Es **clickeable** → `/sales/customers` filtrado. La lista de clientes (`customers.service.findAll` + controller) acepta `createdFrom`/`createdTo` (anclados a Caracas) y `purchased=true|false`; en la UI se muestra un chip del rango + sub-filtro **Todos / Ya compraron / Sin comprar** para ver quiénes no han comprado.
- **Crear producto: sugerencia de nombre (plantilla)** (`components/product-name-suggest.tsx` + `product-form-modal` + `catalog/products/new`): el campo Nombre autocompleta desde productos existentes (reusa `/products?search=`); al elegir copia nombre + categoría, IVA, ganancias, brecha y unidades como plantilla (editable). Estandariza nomenclatura y ayuda a la búsqueda. Solo en modo crear. Puro frontend.
- **Recibo de cobro: cruce a cero** (`receipts/new` + `receipts.service` + schema): cruzar una factura con su devolución (neto = 0) ya se puede procesar. `ReceivablePayment.methodId`/`PayablePayment.methodId` pasan a **nullable** (migración `DROP NOT NULL`); `post()` pasa `methodId || null`; el frontend, si el neto es cero, postea directo con `payments: []` (sin modal, no toca caja). El arqueo no se afecta (lee `receiptPayment`).

#### Sesión 69 (2026-07-11) — Libro mayor de caja (CashLedger): tabla madre del arqueo
Raíz del descuadre: el arqueo sumaba de **3 fuentes** con 2 mecanismos de atribución (ventas por `cashRegisterId`+ventana de tiempo; el resto por `cashSessionId`). Solución: una **tabla madre** donde cae cada línea que toca caja (como `StockMovement` para inventario).
- **Schema:** `CashLedgerEntry` (`cashSessionId`, `direction` IN/OUT, `amountUsd/Bs`, `currency`, `exchangeRate`, `methodId?`, `isCash`, `sourceType`, `sourceId?`, `reason?`, `createdById?`) + enum `CashDir`. Migración `20260711220000_cash_ledger` (+ `CashMovement.isCash`, `CashRegister.showInPos`, `CompanyConfig.useCashLedger`, `Receivable/PayablePayment.methodId` nullable), todo espejado en `fix-schema.sql`.
- **Helper** `common/cash-ledger.ts` `writeCashLedger(tx, input)`: escribe **una fila dentro del `$transaction`** del documento origen. Lo llaman los 7 flujos: ventas (`SALE_PAYMENT` + `CHANGE` vuelto), recibos (`RECEIPT_COLLECTION/PAYMENT` + `REINTEGRO`), gastos (`EXPENSE`), anticipos cliente/proveedor, y movimientos manuales (`MANUAL`).
- **Bugs de caja arreglados de paso:** gasto tomaba moneda de `dto.amountUsd` sin importar el método → ahora `currency`/`isCash` salen de `method.isDivisa`/`isCash`; anticipos electrónicos (Zelle/transfer) ya **no** golpean la gaveta física (`isCash` del método); caja **"Administración"** (`showInPos=false`) que no aparece en el POS para anticipos electrónicos.
- **Arqueo detrás de flag:** `getSessionSalesData` calcula el esperado viejo (`cashExpectedUsdOld/Bs`, 3 fuentes) **y** el del ledger (`ledgerCashExpectedUsd/Bs` = apertura + suma filas `isCash` con signo). Si `CompanyConfig.useCashLedger` está ON, el ledger **es** el arqueo oficial (lo congela `closeSession` en el snapshot y el sobrante/faltante sale de él). Reversible al instante. Default OFF.
- **Reconstrucción/validación:** `backfillLedger(sessionId)` (idempotente, borra+regenera desde ventas/movimientos/recibos, reproduce el arqueo viejo por construcción) + `backfillAllOpenLedger()`. Página admin **`/cash/ledger`** ("Reconstruir y comparar"): backfill, tabla Actual vs Ledger (Cuadra/No cuadra), toggle del flag (solo si todo cuadra). Validado contra la data real de la grande: **10/10 sesiones cuadran, 0 descuadres**.
- **`/cash/movements`**: se agregaron los **vueltos** (filas `CHANGE`) que faltaban (924 vs 908).
- **Libro mayor (UI)** `/cash/ledger/entries` (sidebar CAJA): lista las filas reales del `CashLedgerEntry` con filtros (caja, cajero, origen, moneda, fechas, métodos, solo-efectivo), 4 KPIs de totales, paginado. Backend `GET /cash/ledger-entries` (+ totales por `groupBy`).
- **Reporte PDF** `GET /cash/ledger-entries-report` (`generateLedgerReport`): **agrupado por método de pago** (junta ventas/vueltos/cobros/pagos/gastos/anticipos/manuales del mismo método), total neto por método = ingresos − egresos (para el cuadre). Los movimientos manuales (sin método) se **fusionan al efectivo de su moneda** (Bs→EFECTIVO, USD→DOLAR, buscando el método `isCash` dinámicamente). Columnas idénticas a `movements-report` (`G_PAY_COLS`: Fecha/Caja/Cajero/**Factura/Cliente/Ref.**/USD/Bs), enriqueciendo cada fila por lotes desde su documento origen.

### Módulo de Bancos (Ses.126) — libro banco + conciliación
- **Propósito:** rastrear el dinero que NO está en caja física (cuentas bancarias Bs/USD/Zelle). Módulo NestJS `bancos` (espejo de `divisas`), independiente. Acceso por permiso de rol `'bancos'`; **opt-in por empresa** con `CompanyConfig.bancosEnabled` (default **false**) que gatea los enganches automáticos.
- **Schema:** `BankAccount` (currency VES/USD, `accountType`, `openingBalance`/`openingBalanceBs`/`openingBalanceUsd`+`openingDate`) y `BankMovement` (`direction` IN/OUT, `amount`+`amountBs`+`amountUsd`, `type`, `reference`, `sourceType`/`sourceId`, `transferGroupId`, `reconciled`/`reconciledAt`/`reconciledById`/`statementDate`). `PaymentMethod.bankAccountId` (método electrónico → 1 cuenta fija). Migración idempotente `20260911150000_bancos_module`. Saldo actual = `openingBalance + Σ(IN−OUT)` (el saldo inicial NO es un movimiento).
- **Helper central** `common/bank-ledger.ts`: `writeBankMovement(tx, e)` (idempotente por `sourceType+sourceId+cuenta+monto`; los MANUAL no se deduplican) y `recordPaymentToBank(tx, opts)` (no hace nada si `bancosEnabled=false` o el método no tiene cuenta). Se llama **junto a cada `writeCashLedger`** en `invoices.pay` (`SALE_PAYMENT`), `receipts.post` (`RECEIPT_COLLECTION/PAYMENT`, independiente de sesión de caja), `expenses` y `customer/supplier-advances`. `invoices.updatePaymentMethods` borra/recrea los movimientos NO conciliados de la factura al editar método.
- **UI:** `/bancos` (resumen + aviso de métodos electrónicos sin cuenta), `/bancos/cuentas` (CRUD), `/bancos/[id]` (libro banco, movimiento manual, traspaso, conciliación check-off), selector de cuenta en `/settings/payment-methods`.
- **Fuera de alcance:** reversas por devolución, importación de estados de cuenta, recarga histórica.
...
- **Ajuste de precios por selección** (`/catalog/price-adjustment`): checkbox por artículo (default desmarcados) para ajustar solo los elegidos, no todo el filtro. Layout rediseñado (filtros arriba + tabla a ancho completo, sin paginación). Backend acepta `productIds` en `POST /products/price-adjustment`. Columna + filtro de Brecha (Sí/No); columnas de mayor reemplazadas por "P. sin IVA".
- **Dashboard del vendedor solo en %** (`/dashboard/seller`): el vendedor no ve montos en $; todo como % de su **meta mensual** (campo `Seller.monthlyGoalUsd`, editable por el vendedor vía `PATCH /dashboard/vendedor/meta`). El backend `getVendedor` devuelve solo porcentajes (ventas % de meta prorrateada por días, devoluciones % de ventas, top productos % de participación). Oculta ticket/pendientes/CxC/neto.
