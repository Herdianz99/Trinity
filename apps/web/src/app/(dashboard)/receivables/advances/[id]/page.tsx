...
export default function CustomerAdvanceDetailPage() {
  ...
  const fetchAdvance = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/proxy/customer-advances/${id}`);
  ...
  useEffect(() => {
    document.title = advance?.customer
      ? `Anticipo ${advance.customer.name} | Trinity ERP`
  ...
  if (error || !advance) {
    return (
      <div className="p-4 lg:p-6 max-w-4xl mx-auto">
        <Link href="/receivables" className="text-slate-400 hover:text-slate-200 text-sm inline-flex items-center gap-1 mb-4">
          <ArrowLeft size={16} /> Volver a CxC
        </Link>
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-5">
        <button onClick={() => router.push('/receivables')} className="text-slate-400 hover:text-slate-200 text-sm inline-flex items-center gap-1 mb-3">
          <ArrowLeft size={16} /> Volver a CxC
        </button>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Anticipo de Cliente</h1>
              <p className="text-slate-400 text-sm">
                {advance.customer?.name || '—'}
                {advance.customer?.rif ? ` · ${advance.customer.rif}` : ''}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => window.open(`/api/proxy/customer-advances/${advance.id}/pdf`, '_blank')}
