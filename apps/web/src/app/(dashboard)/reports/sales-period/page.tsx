...
import { BarChart3, Loader2, Download, Calendar, DollarSign, Receipt, TrendingUp, Award } from 'lucide-react';
...
export default function SalesByPeriodPage() {
  ...
  useEffect(() => {
    document.title = 'Ventas por Periodo | Trinity ERP';
  }, []);
  ...
  const fetchReport = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/proxy/reports/sales-by-period?from=${from}&to=${to}&groupBy=${groupBy}`);
      if (!res.ok) throw new Error('Error al cargar reporte');
      const json: PeriodReport = await res.json();
      setData(json);
      setLoaded(true);
    } catch (err: any) {
  ...
  const exportPdf = () => {
    window.open(`/api/proxy/reports/sales-by-period/pdf?from=${from}&to=${to}&groupBy=${groupBy}`, '_blank');
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Ventas por Periodo</h1>
            <p className="text-sm text-slate-400">Analisis de ventas agrupadas por periodo de tiempo</p>
          </div>
        </div>
        {loaded && data && (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-gradient-to-br from-amber-500/10 to-amber-500/5 border-t-2 border-amber-500 border-x border-b border-slate-700/40 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Mejor Periodo</span>
              </div>
              <p className="text-lg font-bold text-amber-400 truncate">{data.bestPeriod || '---'}</p>
            </div>
          </div>
          ...
          {data.rows.length > 0 && (
            <div className="bg-slate-800/50 border border-slate-700/40 rounded-xl p-4">
              <h2 className="text-lg font-semibold text-slate-100 mb-4">Evolucion de Ventas</h2>
              <ResponsiveContainer width="100%" height={320}>
                <AreaChart data={data.rows} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  <defs>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    <th className="text-left text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">Periodo</th>
                    <th className="text-right text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">Facturas</th>
                    <th className="text-right text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">Subtotal USD</th>
                    <th className="text-right text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">IVA USD</th>
                    <th className="text-right text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">Total USD</th>
                    <th className="text-right text-xs text-slate-400 font-medium uppercase tracking-wider px-4 py-3">Total Bs</th>
                  ...
                  {data.rows.length === 0 ? (
                    ...
                    data.rows.map((row, i) => (
                      <tr key={i} className="border-b border-slate-700/30 hover:bg-slate-800/40">
                        ...
                        <td className="px-4 py-3 text-sm text-slate-300 text-right tabular-nums">${fmt(row.subtotalUsd)}</td>
                        <td className="px-4 py-3 text-sm text-slate-300 text-right tabular-nums">${fmt(row.ivaUsd)}</td>
                        <td className="px-4 py-3 text-sm text-emerald-400 text-right tabular-nums font-medium">${fmt(row.totalUsd)}</td>
                        <td className="px-4 py-3 text-sm text-slate-300 text-right tabular-nums">Bs {fmt(row.totalBs)}</td>
      ...
      {!loaded && !loading && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 text-sm">
            Selecciona un rango de fechas y agrupacion, luego presiona &quot;Generar reporte&quot;
          </p>
        </div>
      )}
    </div>
