...
import { ArrowLeft, ShoppingCart, Loader2, Send, Search, X } from 'lucide-react';
...
export default function NewPurchasePage() {
  ...
  const fetchMeta = useCallback(async () => {
    const [sRes, cfgRes, rateRes] = await Promise.all([
      fetch('/api/proxy/suppliers'),
      fetch('/api/proxy/config'),
      fetch('/api/proxy/exchange-rate/today'),
    ]);
    if (sRes.ok) { const d = await sRes.json(); const list = d.data || d; setSuppliers(list); if (list[0]) setFormSupplier(list[0].id); }
    if (cfgRes.ok) { const cfg = await cfgRes.json(); setFormIslrPct(cfg.islrRetentionPct || 0); }
    if (rateRes.ok) {
      const text = await rateRes.text();
      if (text) { try { const rate = JSON.parse(text); setFormExchangeRate(rate.rate || 1); } catch {} }
    }
  }, []);

  useEffect(() => { document.title = 'Nueva Orden de Compra | Trinity ERP'; }, []);
  ...
  // Calculate costs (convert from BS if needed)
  const itemsWithCosts = formItems.map(item => {
    const rawCost = item.costUsd;
    const costInUsd = formCurrency === 'BS' && formExchangeRate > 0
      ? rawCost / formExchangeRate
      : rawCost;
    ...
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Completa los datos de la orden</p>
        </div>
      </div>
      ...
      <form onSubmit={e => handleSave(e, false)} className="card p-6 space-y-4">
        <div>
        ...
        <div>
          ...
          <div className="space-y-2">
              ...
              return (
                <div key={idx} className="flex gap-2 items-center bg-slate-900/50 rounded-lg p-2">
                  ...
                  <span className="text-xs text-slate-400 w-24 text-right font-mono">
                    ${(item.quantity * finalCost).toFixed(2)}
                  </span>
            ...
            {formItems.length === 0 && <p className="text-sm text-slate-500 text-center py-4">Busca y agrega productos arriba</p>}
          </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-700/50">
          <button type="button" onClick={() => router.push('/purchases')} className="btn-secondary !py-2.5 text-sm">Cancelar</button>
