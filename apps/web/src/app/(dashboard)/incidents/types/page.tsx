...
export default function IncidentTypesPage() {
  ...
  useEffect(() => { document.title = 'Tipos de incidencia | Trinity ERP'; }, []);

  const fetchTypes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/proxy/incidents/types');
      const data = await res.json();
      setTypes(Array.isArray(data) ? data : []);
    } catch {
      setMessage({ type: 'error', text: 'Error al cargar tipos' });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function openEdit(t: IncidentType) { setEditing(t); setFormName(t.name); setModalOpen(true); }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setProcessing(true);
    try {
      const url = editing ? `/api/proxy/incidents/types/${editing.id}` : '/api/proxy/incidents/types';
  ...
  async function handleToggle(id: string) {
    try {
      const res = await fetch(`/api/proxy/incidents/types/${id}/toggle-active`, { method: 'PATCH' });
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Error'); }
      fetchTypes();
    } catch (err: any) {
      setMessage({ type: 'error', text: err.message });
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-100">Tipos de incidencia</h1>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} className="text-center py-12 text-slate-500">No hay tipos</td></tr>
            ) : (
              types.map((t) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-100">{editing ? 'Editar tipo' : 'Nuevo tipo'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} required autoFocus
                  ...
                  placeholder="Ej: Empleado, Robo/Hurto, Cliente…" />
              </div>
