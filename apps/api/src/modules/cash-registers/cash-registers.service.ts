...
export class CashRegistersService {
  ...
  async getLedgerEntries(
    ...
    // Enriquecer cada fila con Factura / Cliente / Ref. segun su documento origen.
    await this.enrichLedgerRows(rows);
  ...
  /**
   * Enriquece filas del ledger con Factura/Cliente/Ref. segun su documento origen (carga por lotes).
   * Muta cada fila agregando docNumber, partyName y reference. Compartido por la lista y el PDF.
   */
  private async enrichLedgerRows(
    rows: Array<{
      sourceType: string; sourceId: string | null; methodId: string | null;
      amountBs: number; reason: string | null;
      docNumber?: string; partyName?: string; reference?: string;
    }>,
  ) {
  ...
  async getLedgerEntriesForReport(filters: {
    ...
    const [rowsRaw, agg, cashMethods] = await Promise.all([
      this.prisma.cashLedgerEntry.findMany({
        ...
        include: {
          ...
          cashSession: { select: { cashRegister: { select: { name: true } }, openedBy: { select: { name: true } } } },
        },
      }),
      this.prisma.cashLedgerEntry.groupBy({
    ...
    for (const g of agg) {
      const su = g._sum.amountUsd || 0, sb = g._sum.amountBs || 0;
      if (g.direction === 'IN') { inUsd += su; inBs += sb; if (g.isCash) { cashInUsd += su; cashInBs += sb; } }
      else { outUsd += su; outBs += sb; if (g.isCash) { cashOutUsd += su; cashOutBs += sb; } }
    }
    const r2 = (n: number) => Math.round(n * 100) / 100;
    const summary = {
      totalRows: rows.length,
      inUsd: r2(inUsd), outUsd: r2(outUsd), netUsd: r2(inUsd - outUsd),
      inBs: r2(inBs), outBs: r2(outBs), netBs: r2(inBs - outBs),
      cashNetUsd: r2(cashInUsd - cashOutUsd), cashNetBs: r2(cashInBs - cashOutBs),
    };
