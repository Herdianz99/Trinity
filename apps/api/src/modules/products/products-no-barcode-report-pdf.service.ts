...
// Reporte vertical (carta) de articulos SIN codigo de barras, agrupado por CATEGORIA.
// Dentro de cada categoria se listan de mayor a menor existencia, para priorizar el
// codificado de lo que hay en stock. Incluye solo los productos que NO tienen barcode.
@Injectable()
export class ProductsNoBarcodeReportPdfService {
  constructor(
  ...
  async generate(onlyWithStock = false): Promise<Buffer> {
    ...
    if (items.length === 0) {
      doc
        ...
        .font('Helvetica')
        .fillColor('#64748b')
        .text('No hay articulos sin codigo de barras.', 40, y + 6, {
    ...
    // Barra de categoria (encabezado de grupo). Salta de pagina si no cabe.
    const drawCategoryRow = (categoryName: string, count: number) => {
      const barHeight = 18;
      ...
      doc.text(categoryName, 46, y + 5, { width: 400 });
    ...
    for (const it of items) {
      if (it.category !== currentCategory) {
        ...
        const count = items.filter((x) => x.category === currentCategory).length;
        drawCategoryRow(currentCategory, count);
      }
      ...
      if (y + rowHeight > doc.page.height - doc.page.margins.bottom) {
        ...
        y = 40;
        drawHeaderRow();
        // Repite el encabezado de la categoria al continuar en la pagina siguiente.
        const count = items.filter((x) => x.category === currentCategory).length;
        drawCategoryRow(`${currentCategory} (cont.)`, count);
      }
