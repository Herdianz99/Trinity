...
- **Frontend**: `PrintMonitor` dibuja variante **"DEVOLUCION"** (titulo, N° de nota, "Factura afectada", firma "Recibido por") en ambos caminos (agente ESC/POS + fallback `window.print()`). Pagina de la nota: boton "Procesar comandas" (POSTED + NCV + MERCHANDISE + no procesada) -> badge "Comandas procesadas" y desaparece "Eliminar". Configuracion -> Areas de Impresion: estrella para marcar la por defecto. Control de Comandas: muestra N° de nota + etiqueta "DEVOL.".
- **Sin tocar**: impresion fiscal de la NC (`sendToFiscalPrinter`) ni recibo de devolucion al cliente (`printReturnReceipt`). Quedan afuera (deferidos): unificar el render agente vs navegador (Tema 1), comandas para NCC/compras, reimpresion de comandas de devolucion.
- API + Web typecheck 0 errores. **Pendiente**: prueba manual en caja/despacho real y deploy.
