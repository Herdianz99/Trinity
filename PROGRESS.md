...
## 🗓️ Sesión 2026-08-13 — Stock paginado, columna Caja en pagos, XML ISLR, anti-doble referencia, franja cliente en POS

> ### ⏳ PENDIENTE DE DEPLOY — HEAD `3a141de` (5 commits, sobre `166fff2`)
> Todo verificado en local (BD = restore de `grande`): typecheck API+Web limpio en cada paso, queries validadas contra la BD real. **Trae 1 migración aditiva** (`20260813140000_payment_method_check_duplicate_ref`, `ADD COLUMN IF NOT EXISTS` + red en `deploy/fix-schema.sql`) — despliegue inofensivo (campo opt-in en `false`). Deploy normal a las 6 empresas: `git pull` + `deploy.sh`.
