...
- **Ticket de venta en Bs (montos e items, no USD)**: a pedido del cliente, el recibo 80mm ahora muestra **todo en Bs** (items, precios unitarios y total), dejando solo la **Tasa** como referencia (se quito el TOTAL USD del cuerpo). En `print-receipt.ts`: nuevo helper `computeReceiptBs(invoice)` que centraliza el calculo y lo usan **ambos** generadores (`buildReceiptHTML` y `buildReceiptText`) para no desincronizarse. **Anti-descuadre (el miedo del cliente)**: el TOTAL sale SIEMPRE de `invoice.totalBs` (campo guardado = lo cobrado, lo que cubren los pagos), nunca de la suma de lineas; y el **residual de redondeo** (1-2 centimos) se carga a la **ultima linea** para que la suma NETA de lineas impresas == (totalBs - IGTF). Asi, si el cliente suma lo que ve, da el total exacto. IGTF se muestra como linea aparte **solo si > 0** (normal en no fiscal = 0). Pagos y vuelto tambien en Bs (usan `amountBs`/`changeBs` guardados). Nuevos helpers `round2` y `fmtBs` (formato `1.234,56`). Typecheck Web limpio. **Necesita deploy web.** (El ticket de DEVOLUCION sigue en USD — pendiente si se quiere igual.)
