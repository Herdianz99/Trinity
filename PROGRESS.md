...
> ### ⏳ PENDIENTE DE DEPLOY — HEAD `44fe4e6` (sobre `166fff2`)
> Todo verificado en local (BD = restore de `grande`): typecheck API+Web limpio en cada paso, queries y flujos validados contra la BD real (varios e2e con JWT/clave dinámica forjados). **Trae 1 migración aditiva** (`20260813140000_payment_method_check_duplicate_ref`, `ADD COLUMN IF NOT EXISTS` + red en `deploy/fix-schema.sql`) — despliegue inofensivo (campo opt-in en `false`). El resto es sin migración; el `fix-schema.sql` además corre un **backfill idempotente** que corrige las descripciones viejas de recibos. Deploy normal a las 6 empresas: `git pull` + `deploy.sh`.
