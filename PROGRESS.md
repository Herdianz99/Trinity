...
> ### ⏳ EN GIT, PENDIENTE DE DEPLOY — HEAD `3eeaadc` (sobre `6783a7d`)
> Tres cambios: dos **solo de frontend** + uno **backend+frontend de solo lectura** (sin migración). Probados en local (typecheck API+Web limpio, página y endpoint responden 200, números validados contra `grande_db`). **Falta desplegar a las 6 empresas.**

- **`3eeaadc` feat(receivables): análisis de plataformas de financiamiento (Cashea/Crediagro)** — nueva vista **"Análisis"** en `/receivables/platforms` con un switch **Análisis / Listado** (el listado existente queda intacto). Endpoint de **solo lectura** `GET /receivables/platforms/analytics?from=&to=` que deriva TODO de las CxC `type=FINANCING_PLATFORM` y su factura de origen — **sin datos nuevos ni migración**. Por plataforma, filtrable por período (rango Caracas sobre `createdAt`): **ventas** (nº, USD, Bs), **cobrado/pendiente + ratio de cobranza**, **% financiado promedio y ponderado + % cuota inicial** (financiado = monto de la CxC; cuota inicial = resto de la factura), **días promedio al 1er abono y a saldar**, **aging por antigüedad** (0-30/31-60/61-90/+90 — estas CxC no tienen `dueDate`, se mide desde `createdAt`; snapshot actual) y **tendencia mensual** (gráfico de barras recharts). Verificado en local contra `grande_db`: Cashea **55.8% financiado** (2320 ventas), Crediagro **40.2%** (42). Archivos: `receivables.service.ts` (+método `platformAnalytics`, 5 `$queryRaw`), `receivables.controller.ts` (+ruta antes de `@Get(':id')`), `receivables/platforms/page.tsx` (vista Análisis + switch). **Sin migración.**
