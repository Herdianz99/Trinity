...
> ### ⏳ Pendiente de deploy — typecheck API+Web limpio + e2e local (grande) verificado y revertido. **CON migración** (aditiva e idempotente). Solo se enciende en **aceros/acerosmayor** (flag `useAlmacenOps`).
...
- **feat(inventario): página "Resumen gerencial" de inventario** (`/inventory/summary`, ítem en el menú INVENTARIO) — panorama para gerentes en una sola llamada `GET /inventory-analysis/manager-summary?from&to` (selector Hoy/7 días/Este mes). 3 bloques elegidos por Diego: **(1) Salud operativa** — Índice 5S promedio + por zona (semáforo), reportes de daño (nº, unidades, **costo de merma $**, merma vs reemplazo); **(2) Alertas y rotación** — valor de inventario, nº bajo mínimo/quiebre, nº stock muerto, top listas (reusa `getRotation`); **(3) Exactitud** — diferencias del último conteo físico aprobado (faltantes/sobrantes en unidades y $, neto). Endpoint agregador server-side (mezcla datos de módulos con permisos distintos), gateado como el resto de `inventory-analysis` (AuthGuard). Verificado contra la grande local. **Sin migración.**
