...
> ### ⏳ EN GIT, PENDIENTE DE DEPLOY — HEAD `a7a3cfb` (sobre `ff0301a`)
> Dos features backend + dos ajustes frontend, **sin migración**. Probados en local (typecheck API limpio, endpoint PDF **200**, bloqueo verificado con **400** contra `grande_db`). **Falta desplegar a las 6 empresas.** (En el mismo lote entra también `62cfdea` fix de barcode nullable de una sesión previa.)
